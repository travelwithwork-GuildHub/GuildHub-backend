...
#### WebSocket 握手 ✅ 已依 P1 裁決一併加上

[app/realtime/manager.py](app/realtime/manager.py) 的
`authenticate_handshake` 現在驗的三件事與 REST 端完全一致：簽章合法、房間
對得上、持有人是自己。**同一種 token 在兩個入口的門檻一樣**，不會有人從
鬆的那邊進來。

**副作用（刻意的）：匿名連線進不了房間。** 匿名身分是伺服器隨機指派的，
對不上任何一張票。這符合 §6.2 —— 票是拿房間密碼向 `/enter` 換來的，本來
就沒有匿名進房這條路。大廳不受影響，仍可匿名。

測試側的連帶改動：`FakeClient` 新增 `session_user` 參數，會用同一把
`SESSION_SECRET` 簽出 Starlette 認得的 session cookie。原本 5 個測試用任意
字串當 user 簽票、靠匿名連線進房，現在改成帶真實身分。驗證路徑是真的，
只是省掉 `/api/login` 那一步（它需要資料庫）。

新增兩個測試釘住新行為：借用別人的票被拒、帶著合法票但沒登入也被拒。
